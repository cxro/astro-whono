...
export const ADMIN_HERO_PRESETS = [
  'default',
  'none',
] as const satisfies readonly HeroPresetId[];
export const ADMIN_HERO_PRESET_SET: ReadonlySet<HeroPresetId> = new Set(
  ADMIN_HERO_PRESETS
);
...
export const ADMIN_SIDEBAR_DIVIDER_SET: ReadonlySet<SidebarDividerVariant> =
  new Set(ADMIN_SIDEBAR_DIVIDER_VARIANTS);
...
export const ADMIN_HOME_INTRO_LINK_DEFAULT = [
  'archive',
  'essay',
] as const satisfies readonly HomeIntroLinkKey[];
export const ADMIN_HOME_INTRO_LINK_LIMIT = 2;
export const ADMIN_HOME_INTRO_LINK_KEY_SET: ReadonlySet<HomeIntroLinkKey> =
  new Set(ADMIN_HOME_INTRO_LINK_KEYS);
...
export const ADMIN_SOCIAL_PRESET_IDS = [
  'github',
  'x',
  'email',
] as const satisfies readonly SiteSocialPresetId[];
export const ADMIN_SOCIAL_PRESET_ORDER_DEFAULT: Record<
  SiteSocialPresetId,
  number
> = {
  github: 1,
...
export const ADMIN_SOCIAL_ORDER_MAX =
  ADMIN_SOCIAL_PRESET_IDS.length + ADMIN_SOCIAL_CUSTOM_LIMIT;
...
export const ADMIN_SOCIAL_ICON_KEY_SET: ReadonlySet<SiteSocialIconKey> =
  new Set(ADMIN_SOCIAL_ICON_KEYS);
...
export const getAdminFooterStartYearMax = (): number =>
...
export const isAdminSidebarDividerVariant = (
  value: string
): value is SidebarDividerVariant =>
  ADMIN_SIDEBAR_DIVIDER_SET.has(value as SidebarDividerVariant);

export const isAdminHomeIntroLinkKey = (
  value: string
): value is HomeIntroLinkKey =>
  ADMIN_HOME_INTRO_LINK_KEY_SET.has(value as HomeIntroLinkKey);

export const isAdminSocialPresetId = (
  value: string
): value is SiteSocialPresetId =>
  (ADMIN_SOCIAL_PRESET_IDS as readonly string[]).includes(value);

export const normalizeAdminSocialIconKey = (
  value: unknown
): SiteSocialIconKey | undefined => {
  if (typeof value !== 'string') return undefined;
  const normalized = value.trim();
  if (!normalized) return undefined;
  if (normalized === 'link' || normalized === 'globe') return 'website';
  return ADMIN_SOCIAL_ICON_KEY_SET.has(normalized as SiteSocialIconKey)
    ? (normalized as SiteSocialIconKey)
    : undefined;
};

export const isAdminSocialIconKey = (
  value: string
): value is SiteSocialIconKey =>
  ADMIN_SOCIAL_ICON_KEY_SET.has(value as SiteSocialIconKey);

export const isAdminSocialOrderValue = (value: number): boolean =>
  Number.isInteger(value) &&
  value >= ADMIN_SOCIAL_ORDER_MIN &&
  value <= ADMIN_SOCIAL_ORDER_MAX;

export const isAdminNavOrderValue = (value: number): boolean =>
  Number.isInteger(value) &&
  value >= ADMIN_NAV_ORDER_MIN &&
  value <= ADMIN_NAV_ORDER_MAX;
...
export const getAdminNavOrderIssues = (
  items: readonly AdminNavOrderInput[]
): AdminNavOrderIssue[] => {
  const orderCounts = new Map<number, number>();
...
export const getAdminHomeIntroLinkOption = (
  ...
  ADMIN_HOME_INTRO_LINK_OPTIONS.find((option) => option.id === id) ??
  ADMIN_HOME_INTRO_LINK_OPTIONS[0];

export const isAdminAllowedHttpsUrl = (
  value: string,
...
const defaultNormalizeCustomSocialLabel = (value: unknown): string =>
  normalizeTrimmed(value);

const cloneCustomItems = (
  items: ReadonlyArray<
    EditableThemeSettings['site']['socialLinks']['custom'][number]
  >
...
): EditableThemeSettings['shell']['nav'] => items.map((item) => ({ ...item }));
...
export const canonicalizeAdminThemeSettings = (
  ...
  const customItems = Array.isArray(socialLinks.custom)
    ? socialLinks.custom
    : [];
  const bitsPage = isRecord(page.bits) ? page.bits : {};
  const bitsDefaultAuthor = isRecord(bitsPage.defaultAuthor)
    ? bitsPage.defaultAuthor
    : {};
  const rawPresetOrder = isRecord(socialLinks.presetOrder)
    ? socialLinks.presetOrder
    : {};
  const rawUiArticleMeta: LooseRecord = isRecord(ui.articleMeta)
    ? ui.articleMeta
    : {};
  ...
  const normalizedCustom: EditableThemeSettings['site']['socialLinks']['custom'] =
    customItems
      .map((item, index) => {
        const record = isRecord(item) ? item : {};
        const iconKey =
          normalizeAdminSocialIconKey(record.iconKey) ??
        ...
        const sortableItem: SortableCustomItem = {
          ...
          order: parseOrder(
            record.order as string | number | null | undefined,
            index + 1
          ),
          __index: index,
        };
  ...
  const normalizedNav = (Array.isArray(shell.nav) ? shell.nav : [])
    .map((item) => {
      ...
      return {
        ...
        order: parseOrder(
          record.order as string | number | null | undefined,
          ADMIN_NAV_IDS.indexOf(id) + 1
        ),
        visible: Boolean(record.visible),
      };
    })
    .filter(
      (item): item is EditableThemeSettings['shell']['nav'][number] =>
        item !== null
    )
    .sort((a, b) => {
      if (a.order !== b.order) return a.order - b.order;
  ...
  return {
    site: {
      ...
      footer: {
        startYear:
          parseInteger(
            siteFooter.startYear as string | number | null | undefined
          ) ?? footerStartYearMax,
    ...
    home: {
      ...
      heroPresetId: isAdminHeroPresetId(rawHeroPresetId)
        ? rawHeroPresetId
        : 'default',
      heroImageSrc,
      heroImageAlt,
    ...
    page: {
      essay: {
        title: normalizeOptionalSingleLine(
          String(isRecord(page.essay) ? (page.essay.title ?? '') : '')
        ),
        subtitle: normalizeOptionalSingleLine(
          String(isRecord(page.essay) ? (page.essay.subtitle ?? '') : '')
        ),
      },
      archive: {
        title: normalizeOptionalSingleLine(
          String(isRecord(page.archive) ? (page.archive.title ?? '') : '')
        ),
        subtitle: normalizeOptionalSingleLine(
          String(isRecord(page.archive) ? (page.archive.subtitle ?? '') : '')
        ),
      },
      bits: {
      ...
      memo: {
        title: normalizeOptionalSingleLine(
          String(isRecord(page.memo) ? (page.memo.title ?? '') : '')
        ),
        subtitle: normalizeOptionalSingleLine(
          String(isRecord(page.memo) ? (page.memo.subtitle ?? '') : '')
        ),
      },
      about: {
        title: normalizeOptionalSingleLine(
          String(isRecord(page.about) ? (page.about.title ?? '') : '')
        ),
        subtitle: normalizeOptionalSingleLine(
          String(isRecord(page.about) ? (page.about.subtitle ?? '') : '')
        ),
      },
    },
    ui: {
      codeBlock: {
        showLineNumbers: Boolean(
          isRecord(ui.codeBlock) ? ui.codeBlock.showLineNumbers : false
        ),
      },
      readingMode: {
        showEntry: Boolean(
          isRecord(ui.readingMode) ? ui.readingMode.showEntry : false
        ),
      },
      articleMeta: {
        showDate:
          typeof rawUiArticleMeta.showDate === 'boolean'
            ? rawUiArticleMeta.showDate
            : true,
        dateLabel: normalizeSingleLine(
          rawUiArticleMeta.dateLabel,
          ADMIN_ARTICLE_META_DATE_LABEL_DEFAULT
        ),
        showTags:
          typeof rawUiArticleMeta.showTags === 'boolean'
            ? rawUiArticleMeta.showTags
            : true,
        showWordCount:
          typeof rawUiArticleMeta.showWordCount === 'boolean'
            ? rawUiArticleMeta.showWordCount
            : true,
        showReadingTime:
          typeof rawUiArticleMeta.showReadingTime === 'boolean'
...
export const validateAdminThemeSettings = (
  ...
  const footerStartYearMax =
    options.footerStartYearMax ?? getAdminFooterStartYearMax();
  ...
  if (!settings.site.description)
    pushIssue('site.description', '站点描述不能为空');
  ...
  if (!settings.site.footer?.copyright) {
    ...
    settings.site.footer.copyright.length > ADMIN_FOOTER_COPYRIGHT_MAX_LENGTH
  ) {
    pushIssue(
      'site.footer.copyright',
      `页脚版权行不能超过 ${ADMIN_FOOTER_COPYRIGHT_MAX_LENGTH} 个字符`
    );
  }
  ...
  if (
    ...
    pushIssue(
      'site.socialLinks.x',
      'X / Twitter 链接只允许 https://x.com/... 或 https://twitter.com/... '
    );
  }
  if (
  ...
  const customLinks = Array.isArray(settings.site.socialLinks?.custom)
  ...
  const presetOrderIssues = new Map<
    SiteSocialPresetId,
    'range' | 'duplicate'
  >();
  ...
  ADMIN_SOCIAL_PRESET_IDS.forEach((id) => {
    const rowLabel =
      id === 'github' ? 'GitHub' : id === 'x' ? 'X / Twitter' : 'Email';
  ...
  if (customLinks.length > ADMIN_SOCIAL_CUSTOM_LIMIT) {
    pushIssue(
      'site.socialLinks.custom',
      `自定义链接最多只能添加 ${ADMIN_SOCIAL_CUSTOM_LIMIT} 条`
    );
  }
  ...
  customLinks.forEach((item, index) => {
    ...
    if (!item.label) {
      pushIssue(
        `${basePath}.label`,
        `自定义链接 #${index + 1} 的显示名称不能为空`
      );
    } else if (item.label.includes('\n') || item.label.includes('\r')) {
      pushIssue(
        `${basePath}.label`,
        `自定义链接 #${index + 1} 的显示名称只允许单行文本`
      );
    }
    ...
    if (!item.href || !isAdminAllowedHttpsUrl(item.href)) {
      pushIssue(
        `${basePath}.href`,
        `自定义链接 #${index + 1} 的链接必须是合法 https:// 地址`
      );
    }
    if (!isAdminSocialIconKey(item.iconKey)) {
      pushIssue(
        `${basePath}.iconKey`,
        `自定义链接 #${index + 1} 的图标必须从白名单中选择`
      );
    }
    ...
    if (typeof item.visible !== 'boolean') {
      pushIssue(
        `${basePath}.visible`,
        `自定义链接 #${index + 1} 的 visible 必须是布尔值`
      );
    }
  });

  if (!settings.shell.brandTitle)
    pushIssue('shell.brandTitle', '侧栏站点名不能为空');
  ...
  if (!settings.home.introLead) {
    ...
    pushIssue(
      'home.introLead',
      `首页导语主文案不能超过 ${ADMIN_HOME_INTRO_MAX_LENGTH} 个字符`
    );
  }
  ...
  if (!settings.home.introMore) {
    ...
    pushIssue(
      'home.introMore',
      `首页导语补充文案不能超过 ${ADMIN_HOME_INTRO_MAX_LENGTH} 个字符`
    );
  }
  ...
  if (!Array.isArray(settings.home.introMoreLinks)) {
    ...
    pushIssue(
      'home.introMoreLinks',
      `首页导语补充链接必须选择 1-${ADMIN_HOME_INTRO_LINK_LIMIT} 个入口`
    );
  } else {
    ...
    settings.home.introMoreLinks.forEach((linkKey, index) => {
      if (!isAdminHomeIntroLinkKey(linkKey)) {
        pushIssue(
          `home.introMoreLinks[${index}]`,
          `首页导语补充链接 #${index + 1} 非法：${String(linkKey)}`
        );
        return;
      }
  ...
  if (
    ...
    const localFilePath = getAdminHeroImageLocalFilePath(
      settings.home.heroImageSrc
    );
    if (
      localFilePath &&
      options.localFileExists &&
      !options.localFileExists(localFilePath)
    ) {
      pushIssue(
        'home.heroImageSrc',
        `Hero 图片指向的本地文件不存在：${localFilePath}`
      );
    }
  }
  ...
  if (!settings.home.heroImageAlt) {
    ...
    settings.home.heroImageAlt.length > ADMIN_HERO_IMAGE_ALT_MAX_LENGTH
  ) {
    pushIssue(
      'home.heroImageAlt',
      `Hero 图片说明不能超过 ${ADMIN_HERO_IMAGE_ALT_MAX_LENGTH} 个字符`
    );
  }
  ...
  pageTitleMap.forEach(([title, label, path]) => {
    ...
    if (title.length > ADMIN_PAGE_TITLE_MAX_LENGTH) {
      pushIssue(
        path,
        `${label} 不能超过 ${ADMIN_PAGE_TITLE_MAX_LENGTH} 个字符`
      );
    }
  });
  ...
  const pageSubtitleMap: Array<[string | null, string, string]> = [
    [
      settings.page.essay?.subtitle,
      '/essay/ 页面副标题',
      'page.essay.subtitle',
    ],
    [
      settings.page.archive?.subtitle,
      '/archive/ 页面副标题',
  ...
  pageSubtitleMap.forEach(([subtitle, label, path]) => {
    ...
    if (subtitle.length > ADMIN_PAGE_SUBTITLE_MAX_LENGTH) {
      pushIssue(
        path,
        `${label} 不能超过 ${ADMIN_PAGE_SUBTITLE_MAX_LENGTH} 个字符`
      );
    }
  });
  ...
  if (settings.page.bits?.defaultAuthor?.avatar) {
    if (
      normalizeAdminBitsAvatarPath(settings.page.bits.defaultAuthor.avatar) ===
      undefined
    ) {
      pushIssue(
      ...
      const localFilePath = getAdminBitsAvatarLocalFilePath(
      ...
      if (
        ...
        pushIssue(
          'page.bits.defaultAuthor.avatar',
          `Bits 默认头像指向的本地文件不存在：${localFilePath}`
        );
      }
    }
  }
  ...
  if (typeof settings.ui?.articleMeta?.showDate !== 'boolean') {
    pushIssue(
      'ui.articleMeta.showDate',
      '文章元信息里的“显示发布日期”必须是布尔值'
    );
  }
  ...
  if (typeof settings.ui?.articleMeta?.dateLabel !== 'string') {
    ...
    pushIssue(
      'ui.articleMeta.dateLabel',
      '文章元信息里的“日期前缀”只允许单行文本'
    );
  } else if (
    settings.ui.articleMeta.dateLabel.length >
  ...
  if (typeof settings.ui?.articleMeta?.showReadingTime !== 'boolean') {
    pushIssue(
      'ui.articleMeta.showReadingTime',
      '文章元信息里的“显示阅读时长”必须是布尔值'
    );
  }

  if (
    !isAdminSidebarDividerVariant(settings.ui?.layout?.sidebarDivider ?? '')
  ) {
    pushIssue(
      'ui.layout.sidebarDivider',
      '侧栏分隔线只允许 默认 / 弱化 / 隐藏'
    );
  }
...
const collectMismatchPaths = (
  ...
  if (Array.isArray(actual) || Array.isArray(expected)) {
    ...
    actual.forEach((item, index) => {
      collectMismatchPaths(
        ...
        expected[index],
        mode,
        appendPathSegment(basePath, `[${index}]`),
        mismatches
      );
    });
    return mismatches;
  ...
  if (isRecord(actual) || isRecord(expected)) {
    ...
    const keys =
      mode === 'exact'
        ? Array.from(new Set([...actualKeys, ...expectedKeys]))
        : actualKeys;
    ...
    keys.forEach((key) => {
      ...
      collectMismatchPaths(
        ...
        expected[key],
        mode,
        appendPathSegment(basePath, key),
        mismatches
      );
    });
    return mismatches;
...
export const getAdminThemeSettingsMismatchPaths = (
  ...
  Array.from(new Set(collectMismatchPaths(actual, expected, mode)));
...
export const createAdminThemeSettingsCanonicalMismatchIssues = (
  ...
  return mismatchPaths.map((path) => {
    ...
    return createValidationIssue(
      normalizedPath,
      `${messagePrefix}：${normalizedPath}`
    );
  });
};

export const getAdminThemeSettingsGroupFileName = (
